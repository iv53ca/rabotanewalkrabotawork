...
class GraphPlotterLog {
public:
    GraphPlotterLog(const std::vector<double>& y, const std::vector<double>& x)
            : y_(y), x_(x) {}

    QChartView* plotGraph1Log(QWidget* parent) {
        if (!parent) {
        ...
        chart->setTitle("Logarithmic plot");

        QLogValueAxis* axisX = new QLogValueAxis();
        axisX->setTitleText("LOG(X)");
